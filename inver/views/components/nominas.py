# inver/views/components/nominas.py
...
class NominasView:
    def _mostrar_nomina_semanal(self):
        ...
        tk.Label(header_inner, text="💰 Nómina", font=('Segoe UI', 16, 'bold'),
                 bg=self.colors['card_bg'], fg='white').pack()
        ...
        tk.Label(sel_frame, text="Trabajador:", font=('Segoe UI', 8, 'bold'),
                 bg=self.colors['card_bg'], fg=self.colors['text_gray']).pack(side='left', padx=(6,3), pady=4)
        ...
        self._crear_boton(btn_frame, "CALCULAR NÓMINA", self._abrir_calculo_nomina, 'primary').pack(side='left', padx=2)
    ...
    def _abrir_calculo_nomina(self):
        ...
        # Guardar el binding_id para poder eliminar el binding correctamente
        def on_root_unmap(e):
            try:
                top.destroy()
            except:
        ...
        header = tk.Frame(main_frame, bg=self.colors['card_bg'], height=45)
        header.pack(fill='x', pady=(0,10))
        header.pack_propagate(False)
        tk.Label(header, text="📊 CÁLCULO DE NÓMINA", font=('Segoe UI', 14, 'bold'),
                 bg=self.colors['card_bg'], fg=self.colors['accent']).pack(expand=True)

        frame_trab = tk.Frame(main_frame, bg=self.colors['card_bg'])
        frame_trab.pack(fill='x', pady=5)
        tk.Label(frame_trab, text="Trabajador:", font=('Segoe UI', 9, 'bold'),
                 bg=self.colors['card_bg'], fg=self.colors['text_gray']).pack(side='left', padx=5)
        combo_trab = ttk.Combobox(frame_trab, font=('Segoe UI', 9), width=40)
        combo_trab.pack(side='left', padx=5, fill='x', expand=True)
        trabajadores = [(ced, t.nombre) for ced, t in self.nomina.trabajadores.items()]
        combo_trab['values'] = [f"{ced} - {nombre}" for ced, nombre in trabajadores]
        seleccion_actual = self.combo_nomina_trabajador.get()
        if seleccion_actual and seleccion_actual in combo_trab['values']:
            combo_trab.set(seleccion_actual)
        elif trabajadores:
            combo_trab.set(combo_trab['values'][0])

        frame_periodo = tk.Frame(main_frame, bg=self.colors['card_bg'])
        frame_periodo.pack(fill='x', pady=5)
        tk.Label(frame_periodo, text="Período:", font=('Segoe UI', 9, 'bold'),
                 bg=self.colors['card_bg'], fg=self.colors['text_gray']).pack(side='left', padx=5)
        rb_quin = tk.Radiobutton(frame_periodo, text="Quincenal (15 días)", variable=periodo_var,
                                 value="Quincenal", bg=self.colors['card_bg'], fg=self.colors['text_light'],
                                 selectcolor=self.colors['input_bg'], activebackground=self.colors['card_bg'])
        rb_quin.pack(side='left', padx=10)
        rb_mens = tk.Radiobutton(frame_periodo, text="Mensual (30 días)", variable=periodo_var,
                                 value="Mensual", bg=self.colors['card_bg'], fg=self.colors['text_light'],
                                 selectcolor=self.colors['input_bg'], activebackground=self.colors['card_bg'])
        rb_mens.pack(side='left', padx=10)

        frame_params = tk.LabelFrame(main_frame, text="Parámetros de Cálculo", font=('Segoe UI', 9, 'bold'),
                                     bg=self.colors['card_bg'], fg=self.colors['accent'], bd=2, relief='groove')
        frame_params.pack(fill='x', pady=5, padx=5)

        row1 = tk.Frame(frame_params, bg=self.colors['card_bg'])
        row1.pack(fill='x', pady=2)
        tk.Label(row1, text="Salario mínimo (Bs):", font=('Segoe UI', 8),
                 bg=self.colors['card_bg'], fg=self.colors['text_gray']).pack(side='left', padx=5)
        entry_sal_min = tk.Entry(row1, textvariable=salario_minimo_var, width=10,
                                 font=('Segoe UI', 8), bg=self.colors['input_bg'], fg=self.colors['text_light'])
        entry_sal_min.pack(side='left', padx=5)
        tk.Label(row1, text="Cestaticket diario (Bs):", font=('Segoe UI', 8),
                 bg=self.colors['card_bg'], fg=self.colors['text_gray']).pack(side='left', padx=5)
        entry_cest = tk.Entry(row1, textvariable=cestaticket_diario_var, width=10,
                              font=('Segoe UI', 8), bg=self.colors['input_bg'], fg=self.colors['text_light'])
        entry_cest.pack(side='left', padx=5)

        row2 = tk.Frame(frame_params, bg=self.colors['card_bg'])
        row2.pack(fill='x', pady=2)
        tk.Label(row2, text="% LOPCYMAT (patronal):", font=('Segoe UI', 8),
                 bg=self.colors['card_bg'], fg=self.colors['text_gray']).pack(side='left', padx=5)
        entry_lop = tk.Entry(row2, textvariable=lopcymat_porc_var, width=6,
                             font=('Segoe UI', 8), bg=self.colors['input_bg'], fg=self.colors['text_light'])
        entry_lop.pack(side='left', padx=5)
        tk.Label(row2, text="% IVSS (patronal):", font=('Segoe UI', 8),
                 bg=self.colors['card_bg'], fg=self.colors['text_gray']).pack(side='left', padx=5)
        entry_ivss_pat = tk.Entry(row2, textvariable=ivss_patronal_porc_var, width=6,
                                  font=('Segoe UI', 8), bg=self.colors['input_bg'], fg=self.colors['text_light'])
        entry_ivss_pat.pack(side='left', padx=5)

        row3 = tk.Frame(frame_params, bg=self.colors['card_bg'])
        row3.pack(fill='x', pady=2)
        tk.Label(row3, text="% IVSS (trab.):", font=('Segoe UI', 8),
                 bg=self.colors['card_bg'], fg=self.colors['text_gray']).pack(side='left', padx=5)
        entry_ivss_trab = tk.Entry(row3, textvariable=ivss_trab_porc_var, width=6,
                                   font=('Segoe UI', 8), bg=self.colors['input_bg'], fg=self.colors['text_light'])
        entry_ivss_trab.pack(side='left', padx=5)
        tk.Label(row3, text="% FAOV:", font=('Segoe UI', 8),
                 bg=self.colors['card_bg'], fg=self.colors['text_gray']).pack(side='left', padx=5)
        entry_faov = tk.Entry(row3, textvariable=faov_porc_var, width=6,
                              font=('Segoe UI', 8), bg=self.colors['input_bg'], fg=self.colors['text_light'])
        entry_faov.pack(side='left', padx=5)
        tk.Label(row3, text="% INCES:", font=('Segoe UI', 8),
                 bg=self.colors['card_bg'], fg=self.colors['text_gray']).pack(side='left', padx=5)
        entry_inces = tk.Entry(row3, textvariable=inces_porc_var, width=6,
                               font=('Segoe UI', 8), bg=self.colors['input_bg'], fg=self.colors['text_light'])
        entry_inces.pack(side='left', padx=5)

        btn_frame = tk.Frame(main_frame, bg=self.colors['bg_main'])
        btn_frame.pack(pady=10)
        btn_procesar = self._crear_boton(btn_frame, "PROCESAR CÁLCULO", lambda: self._procesar_calculo(
            top, combo_trab, periodo_var, salario_minimo_var, cestaticket_diario_var,
            lopcymat_porc_var, ivss_patronal_porc_var, ivss_trab_porc_var, faov_porc_var, inces_porc_var
        ), 'success')
        btn_procesar.pack(side='left', padx=5)

        result_frame = tk.LabelFrame(main_frame, text="Resultados del Cálculo", font=('Segoe UI', 9, 'bold'),
                                     bg=self.colors['card_bg'], fg=self.colors['accent'], bd=2, relief='groove')
        result_frame.pack(fill='both', expand=True, pady=5, padx=5)

        canvas = tk.Canvas(result_frame, bg=self.colors['card_bg'], highlightthickness=0)
        v_scroll = ttk.Scrollbar(result_frame, orient='vertical', command=canvas.yview)
        ...
        labels = {}
        row = 0
        tk.Label(scroll_frame, text="ASIGNACIONES", font=('Segoe UI', 10, 'bold'),
                 bg=self.colors['card_bg'], fg=self.colors['success']).grid(row=row, column=0, columnspan=2, sticky='ew', pady=(5,2))
        row += 1
        for desc in ["Salario de producción", "Cestaticket", "Alícuota utilidades", "Alícuota bono vacacional"]:
            tk.Label(scroll_frame, text=desc+":", font=('Segoe UI', 9),
                     bg=self.colors['card_bg'], fg=self.colors['text_light']).grid(row=row, column=0, sticky='e', padx=5, pady=2)
            lbl = tk.Label(scroll_frame, text="0,00 Bs", font=('Segoe UI', 9, 'bold'),
                           bg=self.colors['card_bg'], fg=self.colors['text_light'])
            lbl.grid(row=row, column=1, sticky='w', padx=5)
            labels[desc] = lbl
            row += 1
        tk.Label(scroll_frame, text="", bg=self.colors['card_bg']).grid(row=row, column=0, columnspan=2, pady=2)
        row += 1
        tk.Label(scroll_frame, text="TOTAL ASIGNACIONES", font=('Segoe UI', 10, 'bold'),
                 bg=self.colors['card_bg'], fg=self.colors['success']).grid(row=row, column=0, sticky='e', padx=5)
        lbl_total_asig = tk.Label(scroll_frame, text="0,00 Bs", font=('Segoe UI', 10, 'bold'),
                                  bg=self.colors['card_bg'], fg=self.colors['success'])
        lbl_total_asig.grid(row=row, column=1, sticky='w', padx=5)
        labels["Total Asignaciones"] = lbl_total_asig
        row += 1

        tk.Label(scroll_frame, text="", bg=self.colors['card_bg']).grid(row=row, column=0, columnspan=2, pady=5)
        row += 1

        tk.Label(scroll_frame, text="DEDUCCIONES", font=('Segoe UI', 10, 'bold'),
                 bg=self.colors['card_bg'], fg=self.colors['error']).grid(row=row, column=0, columnspan=2, sticky='ew', pady=(5,2))
        row += 1
        for desc in ["IVSS (trabajador)", "FAOV", "INCES"]:
            tk.Label(scroll_frame, text=desc+":", font=('Segoe UI', 9),
                     bg=self.colors['card_bg'], fg=self.colors['text_light']).grid(row=row, column=0, sticky='e', padx=5, pady=2)
            lbl = tk.Label(scroll_frame, text="0,00 Bs", font=('Segoe UI', 9, 'bold'),
                           bg=self.colors['card_bg'], fg=self.colors['text_light'])
        ...
        tk.Label(scroll_frame, text="", bg=self.colors['card_bg']).grid(row=row, column=0, columnspan=2, pady=2)
        row += 1
        tk.Label(scroll_frame, text="TOTAL DEDUCCIONES", font=('Segoe UI', 10, 'bold'),
                 bg=self.colors['card_bg'], fg=self.colors['error']).grid(row=row, column=0, sticky='e', padx=5)
        lbl_total_ded = tk.Label(scroll_frame, text="0,00 Bs", font=('Segoe UI', 10, 'bold'),
                                 bg=self.colors['card_bg'], fg=self.colors['error'])
        lbl_total_ded.grid(row=row, column=1, sticky='w', padx=5)
        labels["Total Deducciones"] = lbl_total_ded
        row += 1

        tk.Label(scroll_frame, text="", bg=self.colors['card_bg']).grid(row=row, column=0, columnspan=2, pady=5)
        row += 1

        tk.Label(scroll_frame, text="NETO A PAGAR", font=('Segoe UI', 12, 'bold'),
                 bg=self.colors['card_bg'], fg=self.colors['accent']).grid(row=row, column=0, sticky='e', padx=5)
        lbl_neto = tk.Label(scroll_frame, text="0,00 Bs", font=('Segoe UI', 12, 'bold'),
                            bg=self.colors['card_bg'], fg=self.colors['accent'])
        lbl_neto.grid(row=row, column=1, sticky='w', padx=5)
        labels["Neto a Pagar"] = lbl_neto
        row += 1

        tk.Label(scroll_frame, text="", bg=self.colors['card_bg']).grid(row=row, column=0, columnspan=2, pady=5)
        row += 1

        tk.Label(scroll_frame, text="APORTES PATRONALES", font=('Segoe UI', 10, 'bold'),
                 bg=self.colors['card_bg'], fg=self.colors['warning']).grid(row=row, column=0, columnspan=2, sticky='ew', pady=(5,2))
        row += 1
        for desc in ["IVSS patronal", "LOPCYMAT"]:
            tk.Label(scroll_frame, text=desc+":", font=('Segoe UI', 9),
                     bg=self.colors['card_bg'], fg=self.colors['text_light']).grid(row=row, column=0, sticky='e', padx=5, pady=2)
            lbl = tk.Label(scroll_frame, text="0,00 Bs", font=('Segoe UI', 9, 'bold'),
                           bg=self.colors['card_bg'], fg=self.colors['text_light'])
        ...
        tk.Label(scroll_frame, text="", bg=self.colors['card_bg']).grid(row=row, column=0, columnspan=2, pady=2)
        row += 1
        tk.Label(scroll_frame, text="TOTAL APORTES PATRONALES", font=('Segoe UI', 10, 'bold'),
                 bg=self.colors['card_bg'], fg=self.colors['warning']).grid(row=row, column=0, sticky='e', padx=5)
        lbl_total_pat = tk.Label(scroll_frame, text="0,00 Bs", font=('Segoe UI', 10, 'bold'),
                                 bg=self.colors['card_bg'], fg=self.colors['warning'])
        lbl_total_pat.grid(row=row, column=1, sticky='w', padx=5)
        labels["Total Aportes Patronales"] = lbl_total_pat
        row += 1

        top.labels = labels
        top.lbl_neto = lbl_neto
        top.lbl_total_asig = lbl_total_asig
        top.lbl_total_ded = lbl_total_ded
        top.lbl_total_pat = lbl_total_pat
        ...
        bottom_btn_frame = tk.Frame(main_frame, bg=self.colors['bg_main'])
        bottom_btn_frame.pack(pady=10)

        btn_exportar = self._crear_boton(bottom_btn_frame, "EXPORTAR EXCEL", 
                                         lambda: self._exportar_calculo_excel(top), 'info')
        btn_exportar.pack(side='left', padx=5)

        btn_cerrar = self._crear_boton(bottom_btn_frame, "CERRAR", top.destroy, 'danger')
        btn_cerrar.pack(side='left', padx=5)

    def _procesar_calculo(self, top, combo_trab, periodo_var, salario_minimo_var,
                          cestaticket_diario_var, lopcymat_porc_var, ivss_patronal_porc_var,
                          ivss_trab_porc_var, faov_porc_var, inces_porc_var):
